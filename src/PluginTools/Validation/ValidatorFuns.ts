...
import {validParameter} from "./stringFuns";
import {PluginDirectory,isPluginDirectory, pluginTypes} from "../PluginTypes";
...
const defaultArrayFromNull = (mayBeArr: any[], errmsg: string) => {
  return isNull(mayBeArr) || isUndefined(mayBeArr) ? [] : isArray(mayBeArr) ? compact(mayBeArr) : new Error(errmsg)
}
...
export const applicationPlugins = (applicationPlugins) => {
  return isArray(applicationPlugins) ? applicationPlugins : new Error('.applicationPlugins must include an array of Pomegranate plugins.')
}

export const applicationMember = (applicationMember) => {
  return applicationMember ? applicationMember : null
}

export const overrides = (overrides) => {
  return isString(overrides) ? overrides : new Error('overrides is not a string.')
}